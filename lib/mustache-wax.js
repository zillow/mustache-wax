...
function MustacheWax(argv, fromCli) {
    ...
    if (this.argv.templateEngine === 'handlebars') {
        this.template = this.argv.moduleTemplate;
    } else {
        this.template = './templates/module-hogan.js'
    }
...
MustacheWax.prototype = {
    ...
    /**
     * Initializing template wrapper function. Default uses lib/templates/modules.js
     *
     * @api private
     */
    _initModuleTemplate: function () {
        ...
        // templateFn is the "normal" template function
        if (this.argv.templateEngine === 'handlebars') {
            this.templateFn = Handlebars.template(precompiled);
        } else {
            this.templateFn = precompiled.render.bind(precompiled);
        }
    },

    /**
     * Iterate through templates and process
     *
    ...
    processTemplate: function (template, root) {
        ...
        if (stat.isDirectory()) {
            ...
            if (this.argv.templateEngine === 'handlebars') {
                ...
                precompiled = Hogan.compile(precompiled, { asString: 1 });
            }
